...
public class MapleCraBowmanBuff extends ArmorItem {
    private static final Map<ArmorMaterial, MobEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffect>())
                    .put(MapleCraBowmanMaterial.CRA, MobEffects.NIGHT_VISION).build();

    public MapleCraBowmanBuff(ArmorMaterial material, EquipmentSlot slot, Properties settings) {
    ...
    private void addStatusEffectForMaterial(Player player, ArmorMaterial mapArmorMaterial,
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addEffect(new MobEffectInstance(mapStatusEffect, 2, 1));
